...
Juniper 10G LR SFP+ transceivers, Juniper part number `740-021309`, may fail to enter active service when installed in Intel X722 network interface card ports on SSR-1300 and SSR-1400 routers.
...
The output below is seen when the X722 upgrade is needed.[^x722-firmware] Please upgrade the NIC firmware in order to capture the correct output, or move the transceiver to a non-X722 port to display the SFP information.
...
Contact HPE technical support and initiate a return material authorization (RMA) request. HPE will replace the affected device.
