...
If the SSR software is not already installed on the Node A VM, refer to the [High Availability Conductor Installation](ha_conductor_install.md) procedure. After the installation is complete, the system will restart and the initializer will launch automatically.
...
Log in to the Node B VM. If the SSR software is not already installed on the Node A VM, refer to the [High Availability Conductor Installation](ha_conductor_install.md) procedure. After the installation is complete, the system will restart and the initializer will launch automatically.
If the software is already installed, launch the initializer from the command line using `initialize128t.`
