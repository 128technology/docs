...
| off-subnet-reverse-arp-mac-learning | boolean | Default: False. When `true`, reverse arp learning for off-subnet IP addresses is supported. The reverse flow uses the mac from the incoming packet as the subnet source IP address. |
