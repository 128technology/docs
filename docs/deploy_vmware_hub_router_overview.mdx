...
|------|-------|-------------|
| 1 | [Create the VMware VM](deploy_vmware_hub_router_vm.md) | Create and configure a VMware ESXi VM to host the Hub VM. |
| 2 | [Install SSR Software and Initialize the Router](deploy_vmware_hub_router_install.mdx) | Boot the ISO, install SSR software, and initialize the VM as a conductor-managed router. |
| 3 | [Find VM NIC PCI Addresses](deploy_vmware_hub_router_pci.mdx) | Identify the WAN and LAN VMXNet3 NIC PCI addresses required for the router configuration. |
| 4 | [Configure the Router on the Conductor](deploy_vmware_hub_router_config.mdx) | Stage the router configuration on the conductor so it can be applied after onboarding. |
| — | [Appendix — Hub Router Configuration](deploy_appendix_vmware_hub_router.mdx) | PCLI configuration examples. |
