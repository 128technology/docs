...
## Overview
The ICMP reachability detection plugin is designed to leverage [module based application id](concepts_appid.md#appid-using-modules). The plugin is designed to work with services that need a simple `hunt` based path prioritization. For example, if the router has two wan interfaces - broadband and lte, the plugin can be set up to use broadband as the primary path and lte as the secondary path. The plugin works by performing a continuous ICMP ping for the configured service(s) over each of the specified paths. A unique application-id based service is generated to represent each of the potential paths. When the primary path is reachable the corresponding primary path service will be activated to route the sessions. In the event of the primary path failure, the secondary path service will be activated and all subsequent sessions will then be routed using the new service.
...
| ------- | ------- | ------------------------------------------------------------ |
|enabled | boolean | Control whether the plugin is to be run on the router |
|plugin-network| ip-prefix | Used to automatically assign ip address for generated configuration as and when needed |
...
| Element | Type    | Description                                                  |
| ------- | ------- | ------------------------------------------------------------ |
| address | ip-prefix | The list of addresses to be associated with the service. These are the same addresses that a user would typically configure as the `service > address` |
|hold-down| integer | The amount of time a path must remain up before considering it as reachable.|

The following assumptions/considerations should be made for defining the service properties:

- The `service > name` is used to auto-generate other services so users should pick short names. Failure to do so will result in validation errors for generated services.
- The `service > service-policy > lb-strategy` is assumed to be `hunt` and is recommended to be set to that for clarity. Even if the policy is set to `proportional`, the plugin will not operate in that mode.
...
| Element | Type    | Description                                                  |
| ------- | ------- | ------------------------------------------------------------ |
| path-name | string | The name to uniquely identify a single path. |
| vector-priority | integer | Priority value for the paths with the vector. Lower the value, the better the path priority. |
| probe-address | ip-address | Address of the server to be used to perform ICMP probe towards destination to detect reachability. If the service has a single `/32` address this configuration is not required |
| probe-interval | seconds | Duration of how often to perform an ICMP probe test to the probe target |
| number-of-retries | integer | Number of consecutive missed ICMP ping responses from the destination within the retry-interval before deciding that the path is unusable.|
| retry-interval | seconds | Duration within which to reach the destination. Each attempt will be made in duration / number-of-retries interval |

The following assumptions/considerations should be made while configuring the service-route for the plugin
- Each `service route > next-hop > icmp-probe > path-name` should repesent a single path on the router. This will be used for generating KNIs, tenants and other configurations required to make the implementation work. Failure to do so will result in the auto config generation to fail.
...
- When the `icmp-probe > probe-address` is configured for a larger prefix (such as 0.0.0.0/0), the reachability fate of the entire service and path is tied to that single server. This could cause some false positives in reporting the path as down if the upstream server is facing some issues.

### Auto-Generated Configuration

The `service > icmp-probe` and `service-route > icmp-probe` configuration will be used to generate other functional configuration elements as needed. On a high level configuration needs to be generated to do the following:
...
The application module JSON will be set up based on the `service > icmp-probe` configuration to learn the service-address and transport. For the example above, the JSON for the primary path will look like this:
...
The diagram below shows two available paths - broadband and lte. In a sunny day scenario, the icmp probe plugin will perform continuous pings to the destination and report the status as up. In this case, the configured `service > icmp-probe > service-address` will be configured in the application module for broadband service. This in turn will trigger the 128T router to install the FIB entries for the `internet-broadband` service and all incoming sessions will be routed to the only available path for that service called `rte-internet-broadband`. This is essentially why the plugin can only operate in hunt mode.
...
### Service Status
The plugin relies various `systemd`services to work in cohort to provide a robust solution.
