...
|------|-------|-------------|
| 1 | [Configure the VPC](deploy_gcp_conductor_prereqs.mdx) | Create the required VPC, subnet, and SSH key pair in GCP |
| 2 | [Create the GCP VM](deploy_gcp_conductor_install.mdx) | Launch the conductor VM using the Marketplace GUI or CLI (Terraform) |
...
- **SSH key pair** — an RSA key pair for authenticating to the conductor VM. Steps to create one are in [Step 1 — Configure the VPC](deploy_gcp_conductor_prereqs.mdx).
