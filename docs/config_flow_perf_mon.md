...
Flow Performance Monitoring (FPM) operates on top of existing traffic, and does not initiate its own traffic. Because of the interval timing and sample time, no significant load is added to the path. In `monitor-mode true`, no impact to load balancing is generated.  
...
- `peer-path loss`: Packet loss percentage for the SSR peer path.
- `peer-path mos`: MOS value calculated for the SSR peer path (hundredths of a decimal).
