...
```
admin@conductor-east-1.RTR_EAST_CONDUCTOR (service-policy[name=resiliency-policy])# do show service-path service-name east router combo-west node combo-west-a
...
Looking at the configuration snippet carefully, you can see that  `svr-primary` and `svr-backup` did not specify a priority value. The values assigned to these vectors will be determined by their index in the list.
e.g.,
```
local-breakout - index 0
svr-primary    - index 1 
...
Following from this, `svr-primary` will have a value of 10^1 and `svr-backup` will have a value of 10^3.
The vector values translated from this configuration will be:
