...
This guide describes the process for deploying a Conductor and a Session Smart Router (SSR) in Azure.
...
* The existing VNet should be segmented with at least one subnet.
  ...
  * The 128T Routers managed by this Conductor must be able to reach the IP address of Conductor in this subnet.
...
When deploying the Conductor using the templates referenced later in this section, the following infrastructure elements are created automatically on your behalf in order to assist you with the deployment process:
* Virtual machine using an SSR image available in the marketplace.
...
* Depending on the template selected the Conductor application or the 128T installer will be installed.
...
Provide it in the "Instance Name" field (for example: SSRConductor).
...
Once the deployment completes, information about the new Conductor deployment is provided in the Outputs tab placed on the left hand side. Click on the HTTPs URL to login to the  Conductor GUI (if the web browser used is Chrome and it does not trust the connection to the  Conductor GUI due to its self-signed certificate type: thisisunsafe). The credentials are "admin" for username and the name of the VM for the password. To login to the VM via SSH use the username and the SSH public key provided in the template.
...
Once the deployment completes, information about the new Conductor deployment is provided in the Outputs section. Click on the HTTPs URL to login to the Conductor GUI (if the web browser used is Chrome and it does not trust the connection to the Conductor GUI due to its self-signed certificate type: thisisunsafe). The credentials are "admin" for username and the password is the name of the VM. To login to the VM via SSH use the username and the SSH public key provided in the template.
...
:::important
Please note that deploying Session Smart Routers without a valid certificate will be limited to deployments within the cloud only. If your use case requires the deployment of a 128T Router on your premises as well please contact Juniper Networks.
:::
...
* Each network interface has a network security group associated. The network security groups are configured in accordance with the requirements to deploy a fabric with SSR.
* The public and management interfaces have a unique and static public IP address associated.
* Depending on the template selected the SSR application or the 128T installer will be installed.
...
Once the deployment completes, information about the new SSR deployment is provided in the Outputs tab placed on the left hand side. To login to the instance via SSH use the username and the SSH public key provided in the template.
...
Once the deployment completes, information about the new SSR deployment is provided in the Outputs section. To login to the VM via SSH use the username and the SSH public key provided in the template.
...
This section describes in greater detail the different 128T images available in the Marketplace. For a quick and easy deployment of the 128T software for Proof Of Concept purposes, please refer to the hourly images that are linked in the first two sections of this document: [128T Conductor Deployment](#128t-conductor-deployment) and [Session Smart Router Deployment](#128t-session-smart-router-deployment).

#### SSR Conductor images

The images available to deploy a 128T Conductor are the following:

* SSR Conductor. No certificate is required to deploy this image, therefore it is the recommended image to use if a certificate not in your possession. The SSR software is billed hourly when running in addition to the cost of running the VM. For additional information about this image please visit the Marketplace [here](https://azuremarketplace.microsoft.com/en-ca/marketplace/apps/128technology.128technology_conductor_hourly).
* 128T Networking Platform. A certificate is required to install the software, therefore it is the recommended image to use if a certificate is in your possession. There is no cost for running the SSR software, the cost of running the VM is the only cost. For more information about the image please visit the Marketplace [here](https://azuremarketplace.microsoft.com/en-us/marketplace/apps/128technology.128t_networking_platform).
* Private. No certificate is required to deploy this image. There is no cost for running the 128T software, the cost of running the VM is the only cost. Access to the SSR software should have been provided to your Azure subscription as a result of a partnership agreement.
...
##### <u>SSR Conductor (Hourly)</u>

Deploy the Conductor using the Conductor (Hourly) image: 
...
5. Select the **SSR Conductor** offering.
...
* Session Smart Router. No certificate from Juniper is required to deploy this image, therefore it is the recommended image to use if a certificate is not in your possession. The 128T software is billed hourly when running in addition to the cost of running the VM. For additional information about this image please visit the Marketplace [here](https://azuremarketplace.microsoft.com/en-ca/marketplace/apps/128technology.128technology_router_100_hourly).
...
Deploy the SSR using the SSR (Hourly) image:
...
Deploy the SSR using the 128T Networking Platform (BYOL) image:
1. Login to the [Azure Portal](https://portal.azure.com).
...
Deploy the SSR using the Private image:
1. Login to the [Azure Portal](https://portal.azure.com).
...
* Applicable when deploying either an SSR Conductor or a Session Smart Router using the SSR Networking Platform image only:
...
* Applicable when deploying an SSR using the 128T Networking Platform image only:
...
This section describes how to fill out and launch the template via the portal and programmatically deploy a Conductor and a Session Smart Router.

#### SSR Conductor

This section describes the parameters to fill out the template to deploy a Conductor as well as how to launch it via the portal and programmatically.
...
| Resource group          | Select an existing resource group or create a new one.                                                                                                                    |
| Location                | The first instance of the Location field will be  filled out automatically with the location corresponding to the resource group on your behalf.                          |
| Instance Name           | Provide a name to the VM for the SSR.                                                                                                               |
...
| Virtual Network Name    | Name of the existing VNet where the SSR is going to be deployed to.                                                                                 |
...
Once the deployment of the template completes, information about the new SSR deployment is provided in the tab Output placed at the left hand side.
