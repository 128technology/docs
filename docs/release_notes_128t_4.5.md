...
- **I95-34650** In a multihop SVR scenario, the system may have incorrectly attributed incoming packets as coming from a different peer path, resulting in packet loss until load-balancer learns of the loss and migrates the session.

  _**Symptom:**_ Show peers will show the physically disconnected peer as UP while this state.
------
...
------
- **I95-36873** Alarms generated by a router in an authority are incorrectly also sent as SNMP traps from all other routers in the authority.
------
