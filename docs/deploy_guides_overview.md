...
Conductor deployments currently include VMware ESXi and Google Cloud Platform (GCP). In each guide the conductor will have a configuration ready for a branch router to onboard and come online, managed by the conductor, forwarding internet traffic for LAN users, and reachable by the conductor over the same WAN interface used for internet breakout.
