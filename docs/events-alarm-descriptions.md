...
| unclear | Category | Description | Threshold / Cause | Troubleshooting |
|---------|----------|-------------------|-------------|-----|
| X | System | Memory exceeded: `{threshold}%` | Threshold is 90% | The SSR tracks memory resources. The alarm may indicate one or more of the following: <br/>- Insufficient memory <br/>- Hardware issue <br/>- Software issue <br/>Contact customer support for assistance. | 
...
| | System | Node `{node}` went offline ||
X| System | `{node}`: Internal Synchronization database is disconnected | | Verify connectivity between hardware nodes is healthy. Check for additional related alarms. If connectivity is present, please contact customer support for additional assistance.|
...
| X | Platform | Configure security key distribution failed for: `{configKeyError}` | | TBD |
...
| | System | No active NTP server ||	
| X | System | Disk space usage exceeds: `{threshold}`% | The threshold is 90% | The SSR tracks disk resources and cleans large files in expected file system locations. This should not happen if there is no software or hardware defect and the systems were properly sized. Please contact customer support. |
| | System | Restart is required for configuration to take effect ||
| | System | The following accounts have default passwords and must be updated with a string user selected password: `{users}`	||
| | System | Corrupt entitlement certificate received ||
| | System | Invalid entitlement certificate received ||
| | System | Unable to obtain entitlement certificate ||
| | System | Unknown entitlement certificate error ||
| | System | The following chassis sensor(s) have exceeded their recommended temperature thresholds, system performance will degrade if the temperature continues to rise: `{sensors}` | SSR-4xx series only ||
...
| | System | SNMP server failure | | |
| X | System | Configuration synchronization state unknown | | TBD |
| X | System | Not synchronizing with a Conductor | | Please check connectivity to conductor, e.g. using "show system connectivity" |
...
| X | System | Received correct configuration update from conductor but unable to commit || This should not happen if there is no software or hardware defect and the systems were properly sized. Please contact customer support. |
| X | System | Received configuration from conductor but unable to commit in local override || A local override has been definied which conflicts a configuration received from conductor. Please review your local override configuration and/or  contact customer support. |
| X | System | Failed to parse configuration from conductor || Please double-check if conductor and router are running the same software release. If they do, please contact customer support. |
| X | System | Failed to load configuration from conductor with router's datamodel || Please double-check if conductor and router are running the same software release. If they do, please contact customer support. |
| X | System | Configuration provided by conductor does not pass local validation || Please double-check if conductor and router are running the same software release. If they do, please contact customer support. |
...
| | Peer | The following certificates are expiring in less than 7 days: `{value}` |||
| | Peer | Peer `{name}` path MTU is unresolvable (`{path}`) |||
| X | Platform | Application script on interface `{name}` exited unexpectedly || This should not happen if there is no software or hardware defect and the systems were properly sized. Please contact customer support. |
...
| X | Platform | Data plane CPU exceeded: `{threshold}`% | The threshold is 85% | The SSR tracks CPU resources. The alarm may indicate one or more of the following: <br/>- Insufficient CPU <br/>- Hardware issue<br/>- Software issue<br/> Contact customer support for assistance.  |
| X | Platform | Traffic Engineering CPU exceeded: `{threshold}`% | The threshold is 85% | The SSR tracks CPU resources. The alarm may indicate one or more of the following: <br/>- Insufficient CPU <br/>- Hardware issue<br/>- Software issue<br/> Contact customer support for assistance.  |
| X | Platform | `{table}` capacity exceeded: `{threshold}`% | The threshold for the tables is 95%. The tables include: Flow, FIB, Action, ARP, Source Tenant, Access Policy. | The alarm is cleared when 80% or less of the total table is utilized. This may be due to suboptimal configuration or insufficient memory allocated to the SSR software. Contact Juniper support if this alarm persists. |
...
| X | Service | `{node}`: SessionProc CPU usage alarm || This should not happen if there is no software or hardware defect and the systems were properly sized. Please contact customer support. |
| | Process | Process has exited unexpectedly: `{eventProcess}` |||
...
| | MSDP Neighbor | Neighbor `{source}` failed to reach established state |||
...
| Interface	Minor | backup into `{interface}` taken over as primary|||
