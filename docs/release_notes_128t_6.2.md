...
------
- **I95-52540 Metrics infrastructiure resource consumption:** The reporting infrastructure reaching load capacity led to data gaps in custom graphs. Several internal optimizations have been implemented to address this issue. However, to reduce the metrics infrastructure load, metrics in the GUI regarding firmware-generated services, service routes, and tenants will no longer be tracked.
------
...
------
- **I95-54856 Rollback issue after upgrade to 6.2 from versions 6.1 or earlier:** If you upgrade from 6.1 (or older) to 6.2 and you have BGP neighbors with address family configuration, and then try to downgrade to a version prior to 6.2, the downgrade will fail.
...
One workaround is to copy off the `neighbor address-family` container and then delete it from the configuration before the downgrade. That configuration will need to be added back after the downgrade.

Another option is to explicitly configure `remove-private-as` to one of the other valid values. It is recommended to review your BGP configuration before making any of the below changes, as they may have an impact. 
