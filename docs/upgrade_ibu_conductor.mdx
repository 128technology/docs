...
:::note
For sequenced upgrades of HA **routers** (initiated from the Conductor), the Conductor selects the upgrade order automatically based on the state of the `routingManager` process: the node running `routingManager` in standby is upgraded first to minimize traffic impact. See [Upgrade Sequence Behavior in HA Routers](upgrade_router.mdx#upgrade-sequence-behavior-in-ha-routers) for details.
:::
