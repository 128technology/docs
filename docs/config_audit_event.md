...
In the above example all the events available on the 128T router are only retained for one day. The `retention` is of type duration and can take values in hours and days; for example, 24h or 1d.
