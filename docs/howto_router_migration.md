...
:::note
Migrating a router to a conductor running a lower version of software is not supported. For example, Conductor Red is running SSR software version 7.1.6 and manages Router Red. Conductor Blue is running SSR software version 7.0.1. Migrating Router Red to Conductor Blue is not supported. In order to migrate Router Red to Conductor Blue, Conductor Blue must be upgraded to software version 7.1.6. See [Upgrading the Conductor](upgrade_ibu_conductor.md) for upgrade procedures. 
:::
