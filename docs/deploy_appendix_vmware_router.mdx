...
The following block shows the `Spoke1` router section as it appears within the authority configuration. This is the router portion only.
...
config
    authority
        ...
        software-access username  (removed)
        software-access token     (removed)
        exit
    exit
exit
```
