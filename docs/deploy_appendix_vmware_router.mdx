...
:::note
The authority-level objects this router depends on (`Internet-Traffic` service, `corp` tenant, and conductor address) must already exist in the running configuration. For the complete authority-level configuration, see [Appendix — Conductor Configuration](deploy_appendix_conductor.mdx).
:::
